...
try:
    ...
    # print("Make sure L5_generate_all_endpoints.py, L3_get_endpoint_details.py, and L1_find_class_header.py are in the springbootplusplus-web_core directory.")
    sys.exit(1)
...
def generate_code_map(cpp_files: List[str], dry_run: bool = False) -> Dict[str, Dict[str, str]]:
    ...
    for file_path in cpp_files:
        ...
        if generated_code and generated_code.strip():
            ...
            print(f"DEBUG: generate_code_map - Processing file: {file_path}")
            if class_info:
                print(f"DEBUG: generate_code_map - class_name: {class_info.get('class_name')}")
                print(f"DEBUG: generate_code_map - interface_name: {interface_name}")
            else:
                print(f"DEBUG: generate_code_map - No class_info found")
...
def generate_includes(code_map: Dict[str, Dict[str, str]], project_root: Optional[str] = None, include_paths: List[str] = None, exclude_paths: List[str] = None) -> List[str]:
    ...
    if project_root is None:
        ...
        script_dir = os.path.dirname(os.path.abspath(__file__))
        project_root = os.path.dirname(os.path.dirname(script_dir))  # Go up from springbootplusplus-web_core/springbootplusplus-web_scripts/ to project root
    ...
    for file_path in sorted(code_map.keys()):
        ...
        print(f"DEBUG: generate_includes - interface_name: {interface_name}")
        
        if not interface_name:
            # Fallback: use implementation header if interface name not found
            print(f"DEBUG: generate_includes - No interface_name found, using implementation header")
        ...
        import io
        import contextlib
        print(f"DEBUG: generate_includes - Searching for interface header: {interface_name}")
        print(f"DEBUG: generate_includes - search_root: {project_root}")
        print(f"DEBUG: generate_includes - include_folders: {include_paths}")
        print(f"DEBUG: generate_includes - exclude_folders: {exclude_paths}")
        
        # Don't suppress stdout for debugging
        ...
        print(f"DEBUG: generate_includes - find_class_header_file returned: {interface_header}")
        ...
        if interface_header:
            ...
            print(f"DEBUG: generate_includes - Interface header not found, using implementation header as fallback")
            file_path_obj = Path(file_path).resolve()
            include_path = str(file_path_obj).replace('\\', '/')  # Use absolute path
            includes.append(f'#include "{include_path}"')
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python springbootplusplus-web_core/L6_generate_code_for_all_sources.py --include src platform                    # Process all files in src and platform
  python springbootplusplus-web_core/L6_generate_code_for_all_sources.py --include src --exclude platform/arduino  # Process with exclude
  python springbootplusplus-web_core/L6_generate_code_for_all_sources.py --include src platform --dry-run          # Dry run to see what would happen
  python springbootplusplus-web_core/L6_generate_code_for_all_sources.py --dry-run                                 # Dry run on all files in current directory
        """
    )
