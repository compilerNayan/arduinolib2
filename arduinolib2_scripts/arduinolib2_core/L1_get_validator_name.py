...
try:
    ...
    debug_print("Error: Could not import required modules. Make sure check_validate_macro.py and find_class_names.py are in the same directory.")
    sys.exit(1)
...
def get_validator_name(file_path: str) -> Optional[str]:
    ...
    if not class_names:
        debug_print(f"Warning: No classes found in {file_path}")
        return None
    ...
    for macro_info in validate_macros:
        ...
        if macro_text == 'VALIDATE':
            ...
            debug_print(f"Found VALIDATE macro without parameter")
            debug_print(f"Class name: {class_name}")
            debug_print(f"Validator name: {validator_name}")
            ...
            if param_start > 0 and param_end > param_start:
                ...
                debug_print(f"Found VALIDATE_WITH macro with parameter: {macro_text}")
                debug_print(f"Class name: {class_name}")
                debug_print(f"Validator name: {validator_name}")
                return validator_name
    
    # If we get here, something went wrong
    debug_print(f"Error: Could not determine validator name from VALIDATE macro")
    return None
...
def process_multiple_files(file_paths: list) -> Dict[str, Optional[Dict[str, Any]]]:
    ...
    for file_path in file_paths:
        ...
        if validator_info:
            debug_print(f"✓ VALIDATE macro found")
            debug_print(f"  Class: {validator_info['primary_class']}")
            debug_print(f"  Validator: {validator_info['validator_name']}")
            debug_print(f"  Type: {validator_info['validate_type']}")
        else:
            debug_print("✗ No VALIDATE macro found")
...
def main():
    ...
    if len(valid_files) == 1:
        ...
        if validator_name:
            if args.simple:
                debug_print(validator_name)
            else:
                debug_print(f"\nValidator name: {validator_name}")
        else:
            debug_print("No VALIDATE macro found in file")
    ...
    if args.summary and len(valid_files) > 1:
        ...
        debug_print(f"Files with VALIDATE: {files_with_validate}")
        debug_print(f"Files without VALIDATE: {files_without_validate}")
        
        if args.detailed:
            debug_print(f"\nDetailed results:")
            for file_path, info in results.items():
                if info:
                    debug_print(f"  {file_path}: {info['validator_name']} ({info['validate_type']})")
                else:
                    debug_print(f"  {file_path}: No VALIDATE")
