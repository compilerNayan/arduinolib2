...
def run_script_sequence(file_path: str, include_paths: List[str], exclude_paths: List[str], dry_run: bool = False) -> Dict[str, any]:
    ...
    # Track if we should continue processing (don't return early, just mark errors)
    ...
    try:
        ...
        if dry_run:
            ...
            try:
                ...
                if success:
                    ...
                    results['errors'].append(error_msg)
                    # print(f"✗ {error_msg}")
                    # Don't return here - continue to mark as processed even if there were earlier errors
            except Exception as e:
                error_msg = f"Error processing @Component annotation: {e}"
                results['errors'].append(error_msg)
                # print(f"✗ {error_msg}")
                # Don't return here - continue even if there was an error
        
        # All steps completed successfully (or at least we tried to process the annotation)
        ...
        results['errors'].append(error_msg)
        # print(f"✗ {error_msg}")
        # Even if there was an exception, try to mark the annotation as processed
        try:
            if not dry_run:
                L1_check_component_macro.comment_component_macro(file_path)
        except:
            pass  # Ignore errors when trying to mark as processed after exception
