...
try:
    ...
    debug_print("Error: Could not import required modules. Make sure L1_get_validator_name.py, find_interface_names.py, and find_cpp_files.py are in the same directory.")
    sys.exit(1)
...
def find_validator_header_path(validator_name: str, search_root: str = ".", include_folders: Optional[List[str]] = None, exclude_folders: Optional[List[str]] = None) -> Optional[str]:
    ...
    if not potential_headers:
        debug_print(f"Warning: No header files found for validator '{validator_name}'")
        return None
...
def process_file_with_validator_include(file_path: str, search_root: str = ".", include_folders: Optional[List[str]] = None, exclude_folders: Optional[List[str]] = None, dry_run: bool = False) -> Dict[str, Any]:
    ...
    debug_print(f"Found validator header: {validator_header}")
    ...
    if not dry_run and changes_made:
        try:
            ...
            debug_print(f"Modified file: {file_path}")
        except Exception as e:
...
def process_multiple_files(file_paths: List[str], search_root: str = ".", include_folders: Optional[List[str]] = None, exclude_folders: Optional[List[str]] = None, dry_run: bool = False) -> Dict[str, Dict[str, Any]]:
    ...
    for file_path in file_paths:
        ...
        if result['has_validator']:
            debug_print(f"Validator: {result['validator_name']}")
            if result['validator_header']:
                debug_print(f"Header: {result['validator_header']}")
                if result['changes_made']:
                    debug_print(f"Changes made: {len(result['changes_made'])}")
                    for change in result['changes_made']:
                        debug_print(f"  Line {change['line_number']}: {change['original']} → {change['new']}")
                else:
                    debug_print("No changes needed")
            else:
                debug_print("Validator header not found")
        else:
...
def main():
    ...
    if args.summary:
        ...
        debug_print(f"Files with validator: {files_with_validator}")
        debug_print(f"Files modified: {files_modified}")
        debug_print(f"Files no changes needed: {files_no_changes}")
        debug_print(f"Files with errors: {files_errors}")
